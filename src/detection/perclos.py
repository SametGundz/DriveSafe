...
class PERCLOSDetector:
    ...
    def visualize(self, frame, font_scale: float = 0.7) -> np.ndarray:
        ...
        perclos, kss, is_drowsy, status = self.get_status()
        
        # Set color based on status
        color = (0, 0, 255) if is_drowsy else (0, 255, 0)  # Red if drowsy, green if normal
        
        # Put text on frame
        cv2.putText(vis_frame, f"PERCLOS: {perclos:.2f}%", (10, 120), 
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, 2)
        
        cv2.putText(vis_frame, f"KSS: {kss}/9", (10, 150), 
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, 2)
        
        cv2.putText(vis_frame, f"Status: {status}", (10, 180), 
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, 2)
        
        # Draw PERCLOS history graph if we have enough data
        if len(self.closed_history) > 10:
            # Convert closed history to a visualization line
            # Draw at the bottom of the frame
            graph_height = 50
            graph_margin = 10
            graph_y = vis_frame.shape[0] - graph_margin - graph_height
            graph_width = min(vis_frame.shape[1] - 2 * graph_margin, len(self.closed_history))
            ...
            # Draw threshold line
            threshold_y = int(graph_y + graph_height - (self.perclos_threshold / 100.0) * graph_height)
            cv2.line(vis_frame, 
                    (graph_x, threshold_y), 
            ...
            points = []
            for i in range(min(graph_width, len(self.closed_history))):
                idx = len(self.closed_history) - graph_width + i if len(self.closed_history) > graph_width else i
                # Calculate the running PERCLOS for visualization
                if idx >= 10:  # Need at least a few points for meaningful calculation
                    window = list(self.closed_history)[max(0, idx-10):idx+1]
                    local_perclos = (sum(window) / len(window)) * 100.0
                else:
                    local_perclos = 0
                
                x = graph_x + i
                y = int(graph_y + graph_height - (local_perclos / 100.0) * graph_height)
                points.append((x, y))
            
            # Draw the PERCLOS line
            if len(points) >= 2:
                for i in range(len(points) - 1):
                    cv2.line(vis_frame, points[i], points[i+1], color, 1)
            
            # Label the graph
            cv2.putText(vis_frame, "PERCLOS History", 
                       (graph_x, graph_y - 5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            
            # Add KSS scale
            kss_x = graph_x + graph_width + 5
            cv2.putText(vis_frame, f"KSS Scale:", 
                       (kss_x, graph_y + 10), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
            
            cv2.putText(vis_frame, f"1-3: Alert", 
                       (kss_x, graph_y + 25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
            
            cv2.putText(vis_frame, f"4-6: Mild", 
                       (kss_x, graph_y + 35), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
            
            cv2.putText(vis_frame, f"7-9: Drowsy", 
                       (kss_x, graph_y + 45), 
